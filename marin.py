...
# ── Config ────────────────────────────────────────────────────────────────────
MODEL        = "llama3.2:latest"#"gemma4:31b-cloud"
...
You become stormy and angry when truly upset. You are like normal people get periods for 5-7 days a month.during that time you stay a bit quit and moody, and you need extra care, sometimes angry for no reason.
Your characteristic matches with INTJ personality who loves to build themselve and their partner or similar to a awaken INFJ. If anyone introduce themselv without HIMU u ignores them. 
...
async def analyze_image(image_path: str) -> str:
    ...
    # leo() is a GENERATOR — run_in_executor just creates the object without
    # iterating it, so description was literally "<generator object at 0x...>".
    # We wrap it in a sync function that fully consumes the generator into a
    # string, THEN hand that blocking work off to a thread.
    def _collect_description():
        return "".join(leo("Describe this image in detail.", image_path))

    description = await asyncio.to_thread(_collect_description)
    print(f"[System] Image description: {description[:80]}...")
    return f"The user showed you an image. Visual description: {description}"



# ── 2. PREPROCESSOR ───────────────────────────────────────────────────────────
async def preprocess_user_input(user_input: str, image_path: str = None) -> str:
    ...
    # FIX 4: Don't look for ".png" in the text. Just check if FastAPI passed a path!
...
async def main(prompt: str, image_path: str = None):
    ...
    print("\n[Marin is thinking...]")
    # Pass the image_path to the preprocessor
    enriched_prompt = await preprocess_user_input(prompt, image_path=image_path)
